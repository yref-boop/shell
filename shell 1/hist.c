...
tPos previous (tPos pos, tList list){

    tPos prev_pos; //previous position
    if (pos == list) //if p is at the beginning of the list
        return NULL_TEXT; //if the given item is the first position, there can't be another item before it
    else {
